...
public class GitServiceTests : IDisposable
{
    ...
    public async Task PrepareWorkspaceAsync_CreatesFeatureBranch()
    {
        ...
        var (_, currentBranch, _) = await _sut.RunGitAsync(
            _repoDir, ["rev-parse", "--abbrev-ref", "HEAD"], CancellationToken.None);
    ...
    public async Task PrepareWorkspaceAsync_ConfiguresIdentity()
    {
        ...
        var (_, userName, _) = await _sut.RunGitAsync(
            _repoDir, ["config", "user.name"], CancellationToken.None);
        var (_, userEmail, _) = await _sut.RunGitAsync(
            _repoDir, ["config", "user.email"], CancellationToken.None);
    ...
    [Fact]
    public async Task PrepareWorkspaceAsync_UpdatesOriginWhenUrlChanges()
    {
        await InitBareRemoteAsync();

        // Clone with original remote
        await _sut.PrepareWorkspaceAsync(
            _repoDir, _remoteDir, "main", "nexus/implement/TEST-1");

        // Create a second bare remote with the same content
        var remote2 = Path.Combine(_tempDir, "remote2");
        await RunProcessAsync("git", ["clone", "--bare", _remoteDir, remote2]);

        // Prepare again with different URL — should update origin
        await _sut.PrepareWorkspaceAsync(
            _repoDir, remote2, "main", "nexus/implement/TEST-2");

        var (_, origin, _) = await _sut.RunGitAsync(
            _repoDir, ["config", "--get", "remote.origin.url"], CancellationToken.None);
        Assert.Equal(remote2, origin);
    }

    [Fact]
    public void RedactUrl_StripsUserInfo()
    {
        var redacted = GitService.RedactUrl("https://token@github.com/org/repo.git");
        Assert.DoesNotContain("token", redacted);
        Assert.Contains("github.com", redacted);
    }
    ...
    public void RedactUrl_LeavesCleanUrlUnchanged()
    {
        ...
        Assert.Equal(url, GitService.RedactUrl(url));
    }
    ...
    private static async Task RunProcessAsync(string fileName, string[] arguments, string? workingDirectory = null)
    {
        ...
        if (process.ExitCode != 0)
        {
            ...
            throw new Exception($"{fileName} {string.Join(" ", arguments)} failed: {stderr}");
        }
    }
}
